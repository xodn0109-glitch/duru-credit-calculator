...
function renderToCompleteList(res) {
  ...
  if (res.electiveToComplete.length > 0) {
    ...
    // 1학년 교차이수 과목 분리 (semester:null && year:1)
    const CROSS_Y1_IDS = new Set(['MU_1','AR_1','TH_1','IN_1','CA_1','EC_1']);
    const crossY1 = res.electiveToComplete.filter(s => CROSS_Y1_IDS.has(s.id));
    const regular = res.electiveToComplete.filter(s => !CROSS_Y1_IDS.has(s.id));

    // 학년·학기 표시 헬퍼
    function semLabel(s) {
      if (!s.year) return '';
      if (s.semester === null) return `${s.year}학년 교차이수`;
      return `${s.year}학년 ${s.semester}학기`;
    }

    // 1학년 교차이수 과목 경고 블록 (전학생 전용)
    if (isTrans && crossY1.length > 0) {
      const warnBlock = document.createElement("div");
      warnBlock.style.cssText = "background:#fff7ed;border:1.5px solid #fed7aa;border-radius:10px;padding:14px 16px;margin:14px 0 10px";
      const chips = crossY1.map(s =>
        `<span class="subj-chip" style="background:#ffedd5;color:#9a3412;border:1px solid #fed7aa">` +
        `${s.name} (${s.credits}학점) · <em style="font-style:normal;font-size:0.75rem">${semLabel(s)}</em></span>`
      ).join("");
      warnBlock.innerHTML = `
        <div style="font-weight:700;font-size:0.85rem;color:#9a3412;margin-bottom:8px">⚠️ 1학년 교차이수 과목 — 별도 협의 필요</div>
        <div style="margin-bottom:8px">${chips}</div>
        <div style="font-size:0.78rem;color:#92400e;line-height:1.6">
          위 과목들은 두루고 1학년 과정에서 1·2학기에 걸쳐 교차 이수하는 과목입니다.<br>
          전입 시기에 따라 이수가 어려울 수 있으며, <b>담당 교사와 별도 협의</b>가 필요합니다.
        </div>`;
      el.appendChild(warnBlock);
    }
